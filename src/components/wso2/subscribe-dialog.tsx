...
export function SubscribeDialog({ open, onOpenChange, baseUrl, api, authService }: SubscribeDialogProps) {
  ...
  const handleCreateApplication = async () => {
    try {
      ...
      const appService = new WSO2ApplicationService(baseUrl, authService)
      const createdApplication = await appService.createApplication(
        newAppName,
        newAppPolicy,
        newAppDescription
      )
  ...
  const handleSubscribe = async () => {
    try {
      ...
      const subscriptionService = new WSO2SubscriptionService(baseUrl, authService)
      const subscription = await subscriptionService.subscribeToApi(
        api.id,
        selectedAppId,
        selectedTier
      )
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Subscribe to this API to get access to its endpoints
          </DialogDescription>
        </DialogHeader>
        ...
        {step === "select-app" && (
          <div className="py-4">
            <RadioGroup value={selectedAppOption} onValueChange={(value) => setSelectedAppOption(value as "existing" | "new")}>
                ...
                <div className="ml-6 mb-4">
                  {applications.length > 0 ? (
                    ...
                    <div className="text-sm text-gray-500">
                      No applications found. Please create a new application.
                    </div>
                  )}
                </div>
              )}
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <p className="text-sm text-gray-500">
                  Select a subscription tier for this API
                </p>
              </div>
            </div>
